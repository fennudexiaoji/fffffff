...
public class WXShareUtil {
    ...
    /**
     * 通过微信分享
     * @param shareContent 分享的内容（文本、图片、链接）
    ...
    public static class ShareContentWebpage extends ShareContent{
        ...
        public ShareContentWebpage(String title, String content,
                                   String url, int picResource,String imgUrl){
            this.title = title;
            this.content = content;
            this.url = url;
            this.picResource = picResource;
     ...
     * 分享链接
     */
    private static final int THUMB_SIZE_SMALL=90;
    private void shareWebPage(final int shareType, ShareContent shareContent) {
        ...
        if(shareContent.getImgUrl()!=null&& !TextUtils.isEmpty(shareContent.getImgUrl())){
            RequestOptions myOptions = new RequestOptions()
                    .override(THUMB_SIZE_SMALL, THUMB_SIZE_SMALL);
            Glide.with(AppApplication.getInstance())
                    .load(BuildConfig.DEFAULT_HOST + shareContent.getImgUrl())
                    .apply(myOptions)
                    .into(new SimpleTarget<Drawable>() {
                        @Override
                        public void onResourceReady(Drawable resource, Transition<? super Drawable> transition) {
                            Bitmap bmp = ((BitmapDrawable) resource).getBitmap();
                            Bitmap thumbBmp = Bitmap.createScaledBitmap(bmp, THUMB_SIZE_SMALL, THUMB_SIZE_SMALL, true);
                            bmp.recycle();
