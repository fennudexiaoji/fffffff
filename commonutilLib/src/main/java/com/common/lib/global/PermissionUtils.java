...
/**
 * 将targetSdkVersion设置为23，
 * 注意，如果你将targetSdkVersion设置为>=23，
 * 则必须按照Android谷歌的要求，动态的申请权限，
 * 如果你暂时不打算支持动态权限申请，则targetSdkVersion最大只能设置为22.
...
//        }
        ...
        try {
            ...
            Toast.makeText(activity, "请开启权限", Toast.LENGTH_SHORT)
                    .show();
    ...
    private static void requestMultiResult(Activity activity, String[] permissions, int[] grantResults, PermissionGrant permissionGrant) {
        ...
        if (notGranted.size() == 0) {//多个权限申请成功
            permissionGrant.onPermissionGranted(CODE_MULTI_PERMISSION);
        } else {
            openSettingActivity(activity, "权限需要被授权,去设置界面手动开启权限");
        }
     ...
     * 一次申请多个权限
     */
    public static void requestMultiPermissions(final Fragment fragment, final Activity activity, String[] requestPermissions, PermissionGrant grant) {

        final List<String> permissionsList = getNoGrantedPermission(fragment,activity,requestPermissions, false);
        final List<String> shouldRationalePermissionsList = getNoGrantedPermission(fragment,activity,requestPermissions, true);
     ...
     * @return
     */
    public static ArrayList<String> getNoGrantedPermission(Fragment fragment, Activity activity, String[] requestPermissions, boolean isShouldRationale) {
        if(fragment!=null){
